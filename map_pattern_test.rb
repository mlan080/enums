...
class MapPatternTest < Minitest::Test
  ...
  def test_squares
    numbers = [1, 2, 3, 4, 5]
    squares = [] #create empty array
    numbers.each do |number| #iterate through numbers array, passed through an argument, squaring each element
      squares << number ** 2 #push into new array using <<
    end
  ...
  def test_normalize_zip_codes
    ...
    numbers.each do |number|
      ...
      #zip_codes << number.sprintf("%#o", number)
  ...
  def test_trim_last_letter
    ...
    trimmed = []
    animals.each {|animal| trimmed << animals.chop }
